 ...
 * 
 * Notice:
 * 1.此D1的wifi模块有点问题，连接速度特别慢，需要延长时间到1分钟左右才可以连接上，而且只能连接Tenda那个路由器(channel)，查找路由SSID时也特别慢，所以该程序调整为3秒一个循环。
 * 2.D1使用12的GPIO接口。这个D1的接口有故障，通过digitalWrite(output1Pin, HIGH)或者LOW已经无法控制数字口了，只能使用“pinMode(output1Pin, OUTPUT);.output:1, input:0”来控制GPIO口。

 * 3.编译时选择开发板为“esp8266--LOLIN(WeMos)D1 R1”
 */
...
const char* myName = "BMS_2";
// const int resetPerSecond = 60;//对其它3块开发板重启的时间，每隔这么多秒就重启一次。
...
void loop() {
  ...
  float voltage = sensorValue * (5.0 / 1023.0) * 3.18;
...
//以下这段为测试低电平触发器的工作状态
  // if(timeCount % 5 == 0){
  //   digitalWrite(output1Pin, LOW);Serial.printf("output1Pin to LOW: %d\n",output1Pin);
  // }else if((timeCount-1) % 5 == 0){
  //   digitalWrite(output1Pin, HIGH);Serial.printf("output1Pin to HIGH: %d\n",output1Pin);
