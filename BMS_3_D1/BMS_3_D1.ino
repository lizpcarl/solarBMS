 ...
 * 
 * Notice:
 * 1.WeMos板子的问题，无法支持4路继电器，最后试验结果是使用D4、D2、D10、D14四个接口来控制，其中D4对应1号位继电器，其余的（D2、D10、D14）依赖D2接口的设备信号。
...
const char* myName = "BMS_3";
const int resetPerSecond = 1860;//对其它3块开发板重启的时间，每隔这么多秒就重启一次。
...
void loop() {
  ...
  if(timeCount++ > resetPerSecond){
    pinMode(output2Pin, OUTPUT);//digitalWrite(output2Pin, LOW);
    // digitalWrite(output3Pin, LOW);
    if(timeCount > resetPerSecond+3){
      pinMode(output2Pin, INPUT);//digitalWrite(output2Pin, HIGH);
      // digitalWrite(output3Pin, HIGH);
      timeCount = 0;
    }
  }
